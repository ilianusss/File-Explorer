            ...
            match menu_item_clone.get_label().unwrap().as_str() {
                ...
                "Compress" => {
                    ...
                    if let Ok(metadata) = fs::metadata(output_file.as_str()) {
                        ...
                        let extension = output_file.split('.').last().unwrap_or_default();
                        let mut parts: Vec<&str> = output_file.split('.').collect();
                        parts.insert(parts.len()-1, "compressed");
                        let new_filename = parts.join(".");
                        match extension {
                            "mp4"|"webm"|"mkv" => compression::compress_video(&output_file, &new_filename),
                            _ => println!("Can't compress that file")
                        } 
